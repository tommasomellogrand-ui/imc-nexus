(function(){
  ...
  function playerLine(p){
    var annotations=[];
    if(number(p.goals))annotations.push("⚽ "+number(p.goals));
    if(number(p.assists))annotations.push("A "+number(p.assists));
    if(number(p.yellow_cards))annotations.push("🟨");
    if(number(p.red_cards))annotations.push("🟥");
    if(p.captain)annotations.push("C");
    if(p.man_of_match)annotations.push("MVP");
    return `<div class="nx-mr-player"><div class="nx-mr-player-slot">${esc(p.lineup_slot||"")}</div><div class="nx-mr-player-copy"><strong>${esc(p.player_name||"")}</strong><span>${esc(annotations.join(" · ")||((p.lineup_role==="substitute"&&!p.played)?"Panchina":""))}</span></div>${p.rating!=null?`<div class="nx-mr-rating">${esc(p.rating)}</div>`:"<div></div>"}</div>`;
  }

  function lineupTeam(data,side){
    var r=data.report||{},name=side==="home"?(r.home_display_name||r.home_team_name):(r.away_display_name||r.away_team_name);
    var tactics=sideItem(data.tactics,side);
    var players=(data.players||[]).filter(function(p){return p.side===side;});
    var starters=players.filter(function(p){return p.lineup_role==="starter";});
    var subs=players.filter(function(p){return p.lineup_role!=="starter";});
    return `<div class="nx-mr-lineup-team"><div class="nx-mr-lineup-title"><strong>${esc(name)}</strong><span>${esc(tactics.formation||"")}</span></div><div class="nx-mr-subhead">STARTING XI</div><div class="nx-mr-player-list">${starters.map(playerLine).join("")}</div>${subs.length?`<div class="nx-mr-subhead">PANCHINA</div><div class="nx-mr-player-list">${subs.map(playerLine).join("")}</div>`:""}</div>`;
  }

  function yesNo(v){return v===true?"Sì":(v===false?"No":"-");}
  function tacticTeam(data,side){
    var r=data.report||{},t=sideItem(data.tactics,side),name=side==="home"?(r.home_display_name||r.home_team_name):(r.away_display_name||r.away_team_name);
    if(!t||!t.side)return `<div class="nx-mr-tactic-team"><div class="nx-mr-tactic-title"><strong>${esc(name)}</strong><div class="nx-mr-formation">-</div></div></div>`;
    var fields=[
      ["MENTALITÀ",t.mentality],["LARGHEZZA",t.width],["RITMO",t.tempo],["PASSAGGIO",t.passing_style],
      ["ATTACCO",t.attacking_style],["PRESSING",t.pressing],["CONTRASTI",t.tackling_style],["LINEA DIFENSIVA",t.defensive_line],
      ["CONTROPIEDE",yesNo(t.counter_attack)],["MARCATURA STRETTA",yesNo(t.tight_marking)],["REGISTA",t.playmaker_name],["UOMO CHIAVE",t.target_man_name],
      ["CAPITANO",t.captain_name],["RIGORISTA",t.penalty_taker_name],["PUNIZIONI",t.free_kick_taker_name],["CORNER",t.corner_taker_name]
    ];
    return `<div class="nx-mr-tactic-team"><div class="nx-mr-tactic-title"><strong>${esc(name)}</strong><div class="nx-mr-formation">${esc(t.formation||"-")}</div></div><div class="nx-mr-tactic-grid">${fields.map(function(f){return `<div><small>${f[0]}</small><strong>${esc(f[1]==null?"-":f[1])}</strong></div>`;}).join("")}</div></div>`;
  }

  function commentaryMarkup(data){
    var rows=data.commentary||[];
    if(!rows.length)return `<div class="nx-mr-story">Commentary non disponibile.</div>`;
    return `<div class="nx-mr-commentary">${rows.map(function(c){return `<div class="nx-mr-comment"><b>${esc(minuteLabel(c))}</b><p>${esc(cleanComment(c.raw_text,c.minute))}</p></div>`;}).join("")}</div>`;
  }

  function renderReport(data){
    var r=data.report||{},homeName=r.home_display_name||r.home_team_name||"Home",awayName=r.away_display_name||r.away_team_name||"Away";
    var homeScorers=scorerText(data,"home"),awayScorers=scorerText(data,"away");
    var mvp=(data.players||[]).find(function(p){return p.man_of_match;});
    var managerHome=r.home_manager_name||"-",managerAway=r.away_manager_name||"-";
    return `
      <div class="nx-mr-shell">
        <header class="nx-mr-top"><button class="nx-mr-back" type="button" aria-label="Indietro">‹</button><div class="nx-mr-top-copy"><small>GW001 · MATCH REPORT</small><strong>${esc(r.competition_name||"")} · ${esc(r.round_name||r.stage_name||"")}</strong></div><div class="nx-mr-top-spacer"></div></header>
        <main class="nx-mr-body">
          <section class="nx-mr-hero">
            <div class="nx-mr-meta">${esc(formatDate(r.match_date))} · ${esc(r.stadium_name||"Stadio non disponibile")}${r.attendance?" · "+esc(Number(r.attendance).toLocaleString("it-IT"))+" spettatori":""}</div>
            <div class="nx-mr-score-grid"><div class="nx-mr-team"><strong>${esc(homeName)}</strong><small>${esc(managerHome)}</small></div><div class="nx-mr-score">${esc(r.home_score)} - ${esc(r.away_score)}</div><div class="nx-mr-team"><strong>${esc(awayName)}</strong><small>${esc(managerAway)}</small></div></div>
            ${(r.home_penalties!=null&&r.away_penalties!=null)?`<div class="nx-mr-pen">Rigori ${esc(r.home_penalties)} - ${esc(r.away_penalties)}</div>`:""}
            ${(homeScorers||awayScorers)?`<div class="nx-mr-scorers"><div>${esc(homeScorers)}</div><div>${esc(awayScorers)}</div></div>`:""}
          </section>
          <nav class="nx-mr-tabs"><button class="nx-mr-tab is-active" data-mr-tab="overview">OVERVIEW</button><button class="nx-mr-tab" data-mr-tab="lineups">LINEUPS</button><button class="nx-mr-tab" data-mr-tab="tactics">TACTICS</button><button class="nx-mr-tab" data-mr-tab="commentary">COMMENTARY</button><button class="nx-mr-tab" data-mr-tab="info">MATCH INFO</button></nav>
          <div class="nx-mr-panel is-active" data-mr-panel="overview">
            <section class="nx-mr-section"><div class="nx-mr-section-head"><small>NEXUS MATCH STORY</small><h2>La partita</h2></div><div class="nx-mr-story">${matchStory(data)}</div>${mvp?`<div class="nx-mr-mvp"><div><small>MAN OF THE MATCH</small><strong>${esc(mvp.player_name)}</strong></div><b>${esc(mvp.rating==null?"-":mvp.rating)}</b></div>`:""}</section>
            <section class="nx-mr-section"><div class="nx-mr-section-head"><small>MATCH DATA</small><h2>Statistiche</h2></div><div class="nx-mr-stats">${statsMarkup(data)}</div></section>
            <section class="nx-mr-section"><div class="nx-mr-section-head"><small>KEY MOMENTS</small><h2>Timeline</h2></div>${timelineMarkup(data)}</section>
          </div>
          <div class="nx-mr-panel" data-mr-panel="lineups"><section class="nx-mr-section"><div class="nx-mr-section-head"><small>TEAMS</small><h2>Formazioni e voti</h2></div>${lineupTeam(data,"home")}${lineupTeam(data,"away")}</section></div>
          <div class="nx-mr-panel" data-mr-panel="tactics"><section class="nx-mr-section"><div class="nx-mr-section-head"><small>INITIAL SETUP · 0’</small><h2>Tattiche iniziali</h2><p>Solo l’assetto iniziale della partita.</p></div>${tacticTeam(data,"home")}${tacticTeam(data,"away")}</section></div>
          <div class="nx-mr-panel" data-mr-panel="commentary"><section class="nx-mr-section"><div class="nx-mr-section-head"><small>NEXUS BROADCAST</small><h2>Commentary completo</h2></div>${commentaryMarkup(data)}</section></div>
          <div class="nx-mr-panel" data-mr-panel="info"><section class="nx-mr-section"><div class="nx-mr-section-head"><small>MATCH INFO</small><h2>Dettagli partita</h2></div><div class="nx-mr-info"><div><small>COMPETIZIONE</small><strong>${esc(r.competition_name||"-")}</strong></div><div><small>TURNO</small><strong>${esc(r.round_name||r.stage_name||"-")}</strong></div><div><small>DATA</small><strong>${esc(formatDate(r.match_date)||"-")}</strong></div><div><small>STADIO</small><strong>${esc(r.stadium_name||"-")}</strong></div><div><small>SPETTATORI</small><strong>${esc(r.attendance?Number(r.attendance).toLocaleString("it-IT"):"-")}</strong></div><div><small>MANAGER CASA</small><strong>${esc(managerHome)}</strong></div><div><small>MANAGER TRASFERTA</small><strong>${esc(managerAway)}</strong></div></div></section></div>
        </main>
      </div>`;
  }
  ...
  function bindModal(){
    if(!modal)return;
    var back=modal.querySelector(".nx-mr-back");
    if(back)back.addEventListener("click",closeModal);
    modal.querySelectorAll("[data-mr-tab]").forEach(function(button){
      button.addEventListener("click",function(){
        var tab=button.getAttribute("data-mr-tab");
        modal.querySelectorAll("[data-mr-tab]").forEach(function(b){b.classList.toggle("is-active",b===button);});
        modal.querySelectorAll("[data-mr-panel]").forEach(function(p){p.classList.toggle("is-active",p.getAttribute("data-mr-panel")===tab);});
  ...
  function openReport(matchId){
    ...
    modal.className="nx-mr-overlay";
    modal.innerHTML=`<div class="nx-mr-shell"><header class="nx-mr-top"><button class="nx-mr-back" type="button">‹</button><div class="nx-mr-top-copy"><small>GW001 · MATCH REPORT</small><strong>Caricamento</strong></div><div class="nx-mr-top-spacer"></div></header><main class="nx-mr-body"><div class="nx-mr-loading">Caricamento Match Report…</div></main></div>`;
    document.body.appendChild(modal);
    var preBack=modal.querySelector(".nx-mr-back");if(preBack)preBack.addEventListener("click",closeModal);
    api("detail",{matchId:Number(matchId)}).then(function(data){
      if(!modal)return;
      modal.innerHTML=renderReport(data);
      bindModal();
    }).catch(function(err){
      if(!modal)return;
      modal.innerHTML=`<div class="nx-mr-shell"><header class="nx-mr-top"><button class="nx-mr-back" type="button">‹</button><div class="nx-mr-top-copy"><small>GW001 · MATCH REPORT</small><strong>Errore</strong></div><div class="nx-mr-top-spacer"></div></header><main class="nx-mr-body"><div class="nx-mr-error">${esc(err.message||err)}</div></main></div>`;
      var b=modal.querySelector(".nx-mr-back");if(b)b.addEventListener("click",closeModal);
    });
  }

  function onClick(event){
    var row=event.target.closest(".nx-mr-enabled[data-nx-mr-id]");
    if(!row)return;
    if(event.target.closest(".nx-match-entity-link,.nx-match-manager-link,a,button"))return;
    event.preventDefault();
    openReport(row.dataset.nxMrId);
  }

  function startObserver(){
    if(observer)return;
    observer=new MutationObserver(function(){window.clearTimeout(startObserver._t);startObserver._t=window.setTimeout(refreshAnnotations,80);});
