...
#pageRoot>[data-imc-native-page="1"],
#pageRoot>[data-imc-native-detail="1"]{
  margin:0!important;
}
`;
  ...
  function syncActive(id){
    ...
    if(id==="imcPlayerCodexGw"||id==="imcCompetitions"){
      ...
      topButtons.forEach(function(button){
        button.classList.toggle("active",button.getAttribute("data-world-section")===section);
      });
    }

    const bottomButtons=document.querySelectorAll(".nx-bottom [data-page]");
    if(id==="imcTransfers"){
      bottomButtons.forEach(function(button){
        button.classList.toggle("active",button.getAttribute("data-page")==="transfers");
      });
    }
  }
  ...
  function mountTop(el){
    ...
    TOP_IDS.forEach(function(id){
      ...
      if(current&&current!==el&&current.parentElement===root)current.remove();
    });
  ...
  function ownerForDetail(detailId){
    return DETAIL_OWNER[detailId]||"";
  }

  function mountDetail(el){
    const root=pageRoot();
    if(!root||!el)return;
    const ownerId=ownerForDetail(el.id);
  ...
  function closeDetail(detail){
    if(!detail)return;
    const ownerId=ownerForDetail(detail.id);
    const owner=ownerId?document.getElementById(ownerId):null;
    detail.remove();
  ...
  function restore(){
    ...
    Object.keys(DETAIL_OWNER).forEach(function(id){
      const detail=document.getElementById(id);
      if(detail&&detail.parentElement===root)detail.remove();
    });

    TOP_IDS.forEach(function(id){
      const top=document.getElementById(id);
      if(top&&top.parentElement===root)top.remove();
    });
  ...
  function scan(){
    ...
    Object.keys(DETAIL_OWNER).forEach(function(id){
      const el=document.getElementById(id);
      if(el&&el.parentElement===document.body)mountDetail(el);
    });

    const root=pageRoot();
    if(root&&activeTop&&!document.getElementById(activeTop)&&savedContent&&savedHost===root&&root.childElementCount===0){
      restore();
    }
  }
  ...
  document.addEventListener("click",function(event){
    ...
    const top=target.closest("#imcPlayerCodexGw,#imcTransfers,#imcCompetitions");
    if(!top)return;
