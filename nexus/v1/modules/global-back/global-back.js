...
function ensureButton(){
  const sh=shell();
  if(!sh)return null;
  let b=sh.querySelector(":scope > [data-nx-global-back]");
  if(b)return b;
  ...
  const content=sh.querySelector(".gw3-content");
  if(content)sh.insertBefore(b,content);else sh.appendChild(b);
  return b;
}
function sync(){
  raf=0;
  ...
  if(!b)return;
  const sh=shell(),content=sh&&sh.querySelector(".gw3-content");
  const canGoBack=history.length>0&&current&&current.target!=="clubhouse";
  b.hidden=!canGoBack||hasOwnBack(content);
}
