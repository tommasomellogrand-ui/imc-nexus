...
function render(rows){
  ...
  s.container.innerHTML=`<div class="tr-roster">${Array.from(groups.entries()).map(([competitionKey,players])=>`<section class="tr-competition"><h3>${e(competitionKey)}</h3><div class="tr-players">${players.map(p=>`<article class="tr-player"><h4>${e(p.player_name)}</h4><div class="tr-player-stats">${stat("PRESENZE TITOLARE",p.starter_appearances)}${stat("PRESENZE SUBENTRATO",p.substitute_appearances)}${stat("PRESENZE IN PANCHINA",p.bench_appearances)}${stat("MINUTI GIOCATI",p.minutes_played)}${stat("MEDIA VOTO",p.avg_rating)}${stat("ASSIST",p.assists)}${stat("GOL",p.goals)}${stat("MIGLIORE IN CAMPO",p.mom)}${stat("AMMONIZIONI",p.yellow_cards)}${stat("ESPULSIONI",p.red_cards)}</div></article>`).join("")}</div></section>`).join("")}</div>`;
}
