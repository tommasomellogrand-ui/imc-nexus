...
function qualifiedSide(row){const q=nk(row.qualified_team);if(!q)return"";if(q===nk(row.home_team))return"home";if(q===nk(row.away_team))return"away";return""}
...
function logoPath(v){const p=c(v);if(!p)return"";return p.startsWith("/")?p:`/nexus/${p.replace(/^\.\//,"")}`}
function teamLogo(logoMaps,row,side){const world=logoMaps.get(c(row.game_world_id));if(!world)return"";const byName=world.byName,byWorld=world.byWorld;const id=c(row[side+"_sm_club_id"]||row[side+"_sm_world_club_id"]);const name=nk(row[side+"_team"]);const t=(id&&byWorld.get(id))||byName.get(name);return logoPath(t&&t.logo_file)}
function initials(name){const p=c(name).split(/\s+/).filter(Boolean);return upper((p[0]?.[0]||"")+(p[1]?.[0]||p[0]?.[1]||""))||"IMC"}
function logoMarkup(src,name,cls){return src?`<img class="${cls}" src="${e(src)}" alt="${e(name)}">`:`<span class="${cls} ch-feed-logo-fallback">${e(initials(name))}</span>`}
function featuredSide(row){const sides=imcSides(row),q=qualifiedSide(row),winner=c(row.match_winner_side);if(sides.length===1)return sides[0];if(q&&sides.includes(q))return q;if(winner!=="draw"&&sides.includes(winner))return winner;return sides[0]||"home"}
function aggregateText(row){const h=Number(row.home_aggregate_score),a=Number(row.away_aggregate_score);return Number.isFinite(h)&&Number.isFinite(a)?`AGG. ${h}-${a}`:""}
function statusData(row){const sides=imcSides(row),phase=c(row.phase_type),q=qualifiedSide(row),winner=c(row.match_winner_side);if(sides.length>1)return{label:"DERBY IMC",tone:"blue"};const side=sides[0]||"home",win=winner===side,qualified=q===side;if(phase==="final")return win?{label:"CAMPIONE",tone:"green"}:{label:"FINALISTA",tone:"red"};if(phase==="second_leg")return qualified?{label:"QUALIFICATO",tone:"green"}:{label:"ELIMINATO",tone:"red"};if(phase==="single_leg")return win?{label:"QUALIFICATO",tone:"green"}:{label:"ELIMINATO",tone:"red"};if(phase==="first_leg")return{label:"ANDATA",tone:"blue"};if(phase==="group")return{label:"GIRONE",tone:"blue"};return{label:phaseLabel(phase),tone:"blue"}}
function visualLabel(row){const side=featuredSide(row),name=c(row[side+"_imc_manager_name"]||row[side+"_manager_name"]);return name?surname(name):c(row.game_world_id)}
function article(row,logoMaps){const s=story(row),comp=competitionName(row),homeLogo=teamLogo(logoMaps,row,"home"),awayLogo=teamLogo(logoMaps,row,"away"),feature=featuredSide(row),featureLogo=feature==="home"?homeLogo:awayLogo,featureTeam=c(row[feature+"_team"]),agg=aggregateText(row),status=statusData(row),pen=row.decided_on_penalties?`<small class="ch-feed-pen">RIGORI ${e(row.home_penalties)}-${e(row.away_penalties)}</small>`:"";
return `<article class="ch-feed-card" data-feed-status="${e(status.tone)}"><div class="ch-feed-visual"><div class="ch-feed-visual-glow"></div>${logoMarkup(featureLogo,featureTeam,"ch-feed-feature-logo")}<span class="ch-feed-visual-kicker">${e(visualLabel(row))}</span><strong>${e(c(row.game_world_id))}</strong><small>${e(row.competition_type==="International"?"COPPA INTERNAZIONALE":"COPPA DOMESTICA")}</small></div><div class="ch-feed-copy"><div class="ch-feed-copy-meta"><span>🏆 ${e(comp)}</span><span>${e(phaseLabel(row.phase_type))}</span><span>${e(dateLabel(row.match_date))}</span></div><h3>${e(s.headline)}</h3><p>${e(s.body)}</p><div class="ch-feed-copy-foot"><span>${e(c(row.game_world_id))}</span><span>AVVERSARIO ${e(s.opponentType==="Unmanaged"?"SENZA MANAGER":upper(s.opponentType))}</span></div></div><div class="ch-feed-scorebox"><div class="ch-feed-score-teams"><div>${logoMarkup(homeLogo,row.home_team,"ch-feed-score-logo")}<small>${e(row.home_team)}</small></div><strong>${e(row.home_score)}<i>-</i>${e(row.away_score)}</strong><div>${logoMarkup(awayLogo,row.away_team,"ch-feed-score-logo")}<small>${e(row.away_team)}</small></div></div>${pen}${agg?`<b class="ch-feed-agg">${e(agg)}</b>`:""}<span class="ch-feed-status ch-feed-status-${e(status.tone)}">${e(status.label)}</span></div></article>`}
function panel(root){const canvas=root.querySelector(".ch-clubhouse-canvas");if(!canvas)return null;let p=canvas.querySelector("[data-ch-feed-panel]");if(p)return p;p=document.createElement("section");p.className="ch-feed-panel";p.setAttribute("data-ch-feed-panel","");p.hidden=true;p.innerHTML=`<div class="ch-feed-head"><div><span>IMC NEXUS</span><h2>FEED <b>🏆 LE COPPE</b></h2><small>Le notti di Coppa dei manager IMC</small></div><div class="ch-feed-live"><i></i><span>LIVE FEED</span></div></div><div class="ch-feed-list" data-ch-feed-list><div class="ch-feed-loading">Caricamento news…</div></div>`;const nav=canvas.querySelector(".ch-bottom-nav");canvas.insertBefore(p,nav||null);return p}
...
async function loadLogoMaps(client,rows){const ids=[...new Set((rows||[]).map(x=>c(x.game_world_id)).filter(Boolean))],maps=new Map();await Promise.all(ids.map(async id=>{try{const r=await client.rpc("imc_nexus_gateway",{p_action:"gw_teams",p_args:{gameWorld:id}});if(r.error)throw r.error;const byName=new Map(),byWorld=new Map();for(const t of Array.isArray(r.data&&r.data.rows)?r.data.rows:[]){for(const name of [t.club_name,t.display_name])if(c(name))byName.set(nk(name),t);for(const raw of [t.sm_world_club_id,t.sm_club_id])if(c(raw))byWorld.set(c(raw),t)}maps.set(id,{byName,byWorld})}catch(_){maps.set(id,{byName:new Map(),byWorld:new Map()})}}));return maps}
async function load(root){if(loading)return;const p=panel(root),list=p&&p.querySelector("[data-ch-feed-list]"),client=window.__IMC_NEXUS_CLIENT__;if(!list||!client)return;loading=true;list.innerHTML='<div class="ch-feed-loading">Caricamento news…</div>';try{const r=await client.rpc("imc_nexus_gateway",{p_action:"club_house_feed",p_args:{}});if(r.error)throw r.error;const rows=Array.isArray(r.data&&r.data.rows)?r.data.rows:[],logoMaps=await loadLogoMaps(client,rows);list.innerHTML=rows.length?rows.map(row=>article(row,logoMaps)).join(""):'<div class="ch-feed-empty">Nessuna news di Coppa disponibile.</div>'}catch(err){list.innerHTML=`<div class="ch-feed-empty">${e(err.message||"Feed non disponibile")}</div>`}finally{loading=false}}
