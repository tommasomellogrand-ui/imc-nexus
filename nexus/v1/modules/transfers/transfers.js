...
function row(x,label,index){return `<button type="button" class="tf-row" data-tf-index="${index}"><small>${e(x.transfer_date_text||"")} ${x.imc_season?"· S"+e(x.imc_season):""}</small><strong>${e(x.player_name||x.player_id)}</strong><p>${e(x.club_from||"-")} → ${e(x.club_to||"-")}</p>${x.amount_text?`<b>${e(x.amount_text)}</b>`:""}${label?`<em>${label}</em>`:""}</button>`}
...
async function mount(o){if(!o||!o.container||!o.client||!o.worldId)throw Error("Transfers: parametri mancanti");s={...s,...o,worldId:c(o.worldId),mode:o.clubWorldId?"club":"world",rows:[],incoming:[],outgoing:[],activeRows:[]};s.container.innerHTML='<div class="tf-empty">Caricamento Trasferimenti…</div>';await load();s.mode==="club"?renderClub():renderWorld()}
