...
function visualForSide(row,side,name,team){const direct=c(row&&row[`${side}_flag_file`]);if(direct)return direct;if(isNations())return nationFlag(name);return logoPath(team)}
...
function standingTeamCell(name){const team=teamRecord(name,null),visual=isNations()?nationFlag(name):logoPath(team),imgClass=isNations()?"cd-standing-flag":"";return `<div class="cd-standing-team">${visual?`<img class="${imgClass}" src="${e(visual)}" alt="${e(name)}">`:""}<b>${e(name||"—")}</b></div>`}
...
function buildTeamMaps(){s.teamByName=new Map();s.teamById=new Map();for(const t of s.teams){if(c(t.team_id))s.teamById.set(c(t.team_id),t);for(const name of [t.club_name,t.display_name])if(c(name))s.teamByName.set(nk(name),t)}s.managerByWorldId=new Map();s.managerBySmId=new Map();for(const m of s.teamManagers){if(!imcManagerName(m))continue;if(c(m.sm_world_entity_id))s.managerByWorldId.set(c(m.sm_world_entity_id),m);if(c(m.sm_manager_id))s.managerBySmId.set(c(m.sm_manager_id),m)}}
