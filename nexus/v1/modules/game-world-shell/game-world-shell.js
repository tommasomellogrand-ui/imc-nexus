...
function render(){
  ...
  state.container.innerHTML=`<section class="gw-shell" data-gw-shell-version="${VERSION}">
    ...
    <div class="gw-shell-content" data-gw-content><div class="gw-shell-empty">Game World pronto. I moduli funzionali verranno agganciati qui.</div></div>
  </section>`;
}
function mount(opts){
  if(!opts||!opts.container||!opts.worldId)throw new Error("Game World Shell: parametri mancanti");
  state={container:opts.container,worldId:clean(opts.worldId),worldName:clean(opts.worldName)};
  render();
}
...
document.addEventListener("click",e=>{
  const b=e.target.closest&&e.target.closest("[data-gw-section]");if(!b)return;
  document.dispatchEvent(new CustomEvent("nexus:navigate",{detail:{target:"game-world-section",worldId:state.worldId,section:clean(b.getAttribute("data-gw-section"))}}));
});
